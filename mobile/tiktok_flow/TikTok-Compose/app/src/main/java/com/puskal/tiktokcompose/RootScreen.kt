...
fun RootScreen() {
    ...
    val isCommentBottomSheetRoute =
        currentRoute == COMMENT_BOTTOM_SHEET_ROUTE ||
            currentRoute?.startsWith("$COMMENT_BOTTOM_SHEET_ROUTE/") == true

    val isShowBottomBar = when (currentRoute) {
        HOME_SCREEN_ROUTE, INBOX_ROUTE,
        FRIENDS_ROUTE, AUTHENTICATION_ROUTE, MY_PROFILE_ROUTE, null -> true
        else -> false
    } || isCommentBottomSheetRoute
    val darkMode = when (currentRoute) {
        HOME_SCREEN_ROUTE, FORMATTED_COMPLETE_CREATOR_VIDEO_ROUTE, CAMERA_ROUTE, null -> true
        else -> false
    } || isCommentBottomSheetRoute

    if(currentRoute == HOME_SCREEN_ROUTE){
       BackHandler {
