...
fun VideoPlayer(
    ...
    LaunchedEffect(key1 = true) {
        withContext(Dispatchers.IO) {
            val bm = FileUtils.extractThumbnail(
                context.assets.openFd(VideoAsset.assetPath(video.videoLink, video.assetDirectory)), 1
            )
    ...
    if (pagerState.settledPage == pageIndex) {
        val internalPlayer = if (player == null) {
            remember(context) {
        ...
        val exoPlayer = player ?: internalPlayer ?: return
